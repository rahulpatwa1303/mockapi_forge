...
async function findMatchingMock(
...
): Promise<DbMockEndpoint | null> {
    // 1. Get user_id for the api_route_prefix
    const { data: profile, error: profileError } = await supabaseAdmin
        .from('profiles')
        .select('id') // user_id is stored as 'id' in profiles table
    ...
    // 2. Get all mocks for this user
    const { data: userMocks, error: mocksError } = await supabaseAdmin
        .from('mock_endpoints')
        .select('*')
        .eq('user_id', userId)
        .eq('http_method', requestedMethod.toUpperCase());

    if (mocksError || !userMocks) {
        console.error('Error fetching mocks for user:', userId, mocksError);
        return null;
    }

    // 3. Find the best match for the path
    // Normalize requestedPath to ensure it starts with / and has no trailing slash (unless it's just "/")
    let normalizedReqPath = requestedPath;
    if (!normalizedReqPath.startsWith('/')) normalizedReqPath = '/' + normalizedReqPath;
    if (normalizedReqPath.length > 1 && normalizedReqPath.endsWith('/')) {
        normalizedReqPath = normalizedReqPath.slice(0, -1);
    }


    for (const mock of userMocks as DbMockEndpoint[]) {
        // Normalize stored path_template
        let normalizedPathTemplate = mock.path_template;
        if (!normalizedPathTemplate.startsWith('/')) normalizedPathTemplate = '/' + normalizedPathTemplate;
        if (normalizedPathTemplate.length > 1 && normalizedPathTemplate.endsWith('/')) {
            normalizedPathTemplate = normalizedPathTemplate.slice(0, -1);
        }

        const matcher = match(normalizedPathTemplate, { decode: decodeURIComponent });
        const result = matcher(normalizedReqPath);

        if (result) {
            // TODO: Could pass result.params to data generator in the future
            return mock;
        }
    }
...
async function handleRequest(
  ...
  try {
    const mockDefinition = await findMatchingMock(api_route_prefix, requestedMethod, requestedPath);
